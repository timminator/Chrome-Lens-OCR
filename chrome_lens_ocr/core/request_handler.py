...
class LensRequestHandler:
    ...
    def get_next_sequence_ids_for_request(self, is_new_image_payload: bool) -> tuple[Optional[int], int, int]:
        ...
        logger.debug(
            f"RequestHandler: Providing IDs for request: SessionUUID (current): {self.current_session_uuid}, "
            f"Next SeqID: {self.current_sequence_id}, Next ImgSeqID: {self.current_image_sequence_id} (is_new_image: {is_new_image_payload})"
        )
    ...
    async def send_request(self, protobuf_payload: bytes, request_uuid_used: int) -> "LensOverlayServerResponse":
        ...
        for attempt in range(1, self.max_retries + 2):
            try:
                response = await self.client.post(LENS_CRUPLOAD_ENDPOINT, content=protobuf_payload, headers=headers)
                ...
                if server_response_proto.HasField("error") and server_response_proto.error.error_type != 0:
                    ...
                    if attempt <= self.max_retries:
                        ...
                        continue
                    raise LensAPIError(error_msg, status_code=response.status_code, response_body=response_bytes.decode(errors="replace"))
                ...
                if server_response_proto.HasField("objects_response") and server_response_proto.objects_response.HasField("cluster_info"):
                    ...
                    if self.last_cluster_info and self.last_cluster_info.server_session_id:
                        logger.debug(
                            ...
                            f"RoutingInfo available: {self.last_cluster_info.HasField('routing_info')}"
                        )
                else:
                    self.last_cluster_info = None
                ...
                raise LensProtobufError(f"Protobuf response parsing error: {e_parse}", response_body=decoded_for_error) from e_parse
